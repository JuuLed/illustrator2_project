...
echo "Test Controller of User script is running...\n";
...
class UserControllerTest extends TestCase
{
	...
	public function testRegisterFails()
	{
		...
		$userModel->method('register')->willReturn(['error' => 'Registration failed']);
		...
		$userController = new UserController($userModel, $cookieSetter);

		$result = $userController->register('fail', 'test@example.com', 'password');
		$this->assertTrue(is_array($result), "register should return an array.");
		$this->assertArrayHasKey('error', $result, "Array should have 'error' key.");
		$this->assertEquals('Registration failed', $result['error'], "Error message should match.");
	}
	...
	public function testLoginFails()
	{
		...
		$userModel->method('login')->willReturn(['error' => 'Login failed']);
		...
		$result = $userController->login('fail@example.com', 'wrongpassword');
		$this->assertTrue(is_array($result), "login should return an array.");
		$this->assertArrayHasKey('error', $result, "Array should have 'error' key.");
		$this->assertEquals('Login failed', $result['error'], "Error message should match.");
	}
